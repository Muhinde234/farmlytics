...
import { defaultTheme } from '../config/theme'; // ✅ Added this import

// ---------------------- Styled Components (Standard Variant) ----------------------
...
// ---------------------- Styled Components (Header Variant - Modal) ----------------------
...
const ModalContent = styled(View)`
  ...
  shadow-opacity: 0.3;
  shadow-radius: 6px;
  margin-top: ${Platform.OS === 'android' ? 50 : 80}px;
...
const LanguageSelector: React.FC<LanguageSelectorProps> = ({ variant = 'standard', color = defaultTheme.colors.lightText }) => {
              ...
              <ModalContent>
                {languages.map((lang, index) => (
                  <View key={lang.code}>
                    ...
                    >
                      <ModalItemText selected={i18n.language === lang.code}>{lang.label}</ModalItemText>
  ...
  return (
    <StandardLanguageContainer>
      {languages.map((lang) => (
        <StandardLanguageButton key={lang.code} selected={i18n.language === lang.code} onPress={() => changeLanguage(lang.code)}>
          <StandardLanguageText selected={i18n.language === lang.code}>{lang.code.toUpperCase()}</StandardLanguageText>
